...
namespace ArrayBasedAttribute {
	...
	class GoodCaseCreature {
		...
		public : 
			...
			int sum() const {
				return accumulate(begin(abillites), end(abillites),0);
			}
			...
			int max() const {
				return *max_element(begin(abillites), end(abillites));
			}
	};
...
namespace TamplateBasedComposite {
	...
	// 1. 개별객체와 컬렉션 객체 모두에 동일한 인터페이스를 사용가능하도록 template method

	template<typename Self> struct NeuronBase;
	struct Neuron;

	template <typename Self>
	struct NeuronBase {
		template <typename T> void connect_to(T& ohter);
	};
	...
	template<typename Self>
	template <typename T> void NeuronBase<Self>::connect_to(T& other){
