...
namespace ProblemCase{
	...
	struct Point{
		...
		// Point(const float r, const float theta): x(r*cos(theta)), y(r*sin(theta)){};  // 2. Polar 좌표계 :  (Error발생) 구분 불가
...
namespace Factory{
	...
	void do_factory(){
		auto p = PointFactory:: NewPolar(5, 3.14);
	}
};
...
namespace InnerFactory{
	...
	void do_inner_factory(){
		auto p = Point::Factory.NewPolar(5, 3.14);
	}
}
