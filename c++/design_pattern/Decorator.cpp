...
struct BaseDecorator : Notifier {
  ...
  void send(const string& message) override { wrapee.send(message); }
...
class SMSDecorator : public BaseDecorator {
 public:
  ...
  void sendSMS(const string& message) { cout << "[SMS] :" << message << endl; }
...
void main() {
  cout << "[good Case]" << endl;
  TransparentShape c1{ColorShape{Circle(23), "red"}, 128};
  cout << c1.str() << endl;

  cout << "[Bad Case]" << endl;
  ColorShape c2{ColorShape{Circle(23), "red"}, "green"};
  cout << c2.str() << endl;
}
};  // namespace DynamicDecortator
