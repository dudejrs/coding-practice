...
// 다른 모듈이 include 하는 header에는 다음과 같은 필요한 public 인터페이스만
// 적재
struct Person {
  string name;
  void greet();
...
Person::Person(string name) : impl(new PersonImpl) { this->name = name; }
...
struct VectorRenderer : Renderer {
  void render_circle(float x, float y, float radius) override {
    cout << "Drawing a vector circle of radius " << radius << " at (" << x
         << "," << y << ")" << endl;
  }
};
...
struct RasterRenderer : Renderer {
  void render_circle(float x, float y, float radius) override {
    cout << "Rastering circle of radius " << radius << " at (" << x << "," << y
         << ")" << endl;
  }
};
