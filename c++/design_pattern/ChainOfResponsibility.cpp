...
namespace HandlerChain {
	...
	struct BaseHandler : Handler {
		void set_next(Handler* next) override {
			this->next = next;
		};
	...
	struct ConcreteHandler : public BaseHandler {
		...
		bool can_handle(const Request& request) {
			return true;
		}
...
namespace PointerChain {
	...
	void main(){
		...
		cout << "name : "<< goblin.name << "( attack : " <<goblin.attack <<", defense : "<< goblin.defense <<")"<< endl;
		...
		root2.handle();
		cout << "name : "<< goblin2.name << "( attack : " <<goblin2.attack <<", defense : "<< goblin2.defense <<")"<< endl;
...
namespace BrokerChain {
	...
	void main(){
		...
		cout << "name : "<< goblin.name << "( attack : " <<goblin.attack <<")"<< endl;

		{
			DoubleAttackModifier dam{game, goblin};
			cout << "name : "<< goblin.name << "( attack : " <<goblin.get_attack() <<")"<< endl;
