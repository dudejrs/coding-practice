...
struct HtmlElement {
  ...
  HtmlElement(string& name, string& text)
      : name(name), text(text), elements(0) {}
...
void do_fluent_interface_budilder_case() {
  ...
  htmlbuilder.add_children(child_name, words[0])
      .add_children(child_name, words[1]);
  // htmlbuilder2->add_children(child_name,
  // words[0])->add_children(child_name,words[1]);
...
struct EnhancedHtmlElement : HtmlElement {
 ...
 protected:
  EnhancedHtmlElement() {}
  EnhancedHtmlElement(const string& name, const string& text)
      : name(name), text(text) {};
};
...
void how_to_notify_uses_to_use_builder_class() {
  ...
  HtmlElement e = htmlbuilder->add_children(child_name, words[0])
                      .add_children(child_name, words[1]);
...
struct Tag {
 ...
 protected:
  Tag(const std::string& name, const std::string& text)
      : name(name), text(text) {};
  Tag(const std::string& name, const std::vector<Tag>& child)
...
void do_groovy_style() {
  ...
  std::cout << P{IMG{"http://url"}} << std::endl;
}
}  // namespace groovy_style
