...
namespace TreeIterator {
	...
	struct Node {
		...
		Node(const T& value, Node<T>* left, Node<T>* const right) :value(value), left(left), right(right){
			this->left->parent = this->right->parent = this;
		}
		Node(const T& value, const Node<T>&& left, const Node<T>&& right) :value(value), left(new Node<T>(left)), right(new Node<T>(right)){
			this->left->parent = this->right->parent = this;
		}
