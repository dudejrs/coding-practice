...
struct Node {
  ...
  void add_children(Node* n) { children.push_back(n); }
};
...
struct Tree {
  ...
  void insert(Node* parent, Node* cur) { parent->children.push_back(cur); }
...
Tree& initialize(default_random_engine& gen) {
  ...
  for (int i = 0; i < n; i++) nodes.push_back(new Node(nullptr, i));
  ...
  for (int i = 0; i < n / 2; i++) {
    if (2 * i + 1 < n) builder.set_children(nodes[i], nodes[2 * i + 1]);
    if (2 * i + 2 < n) builder.set_children(nodes[i], nodes[2 * i + 2]);
  }
...
int main(void) {
  ...
  for (int it = 0; it < TEST_CASES; it++) {
    ...
    for (auto i : preorder_nodes) cout << i << " ";
    cout << endl;
  }
