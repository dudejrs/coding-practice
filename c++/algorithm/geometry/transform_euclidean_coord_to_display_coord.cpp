...
vector<pair<int,int>> euclidan_to_display(vector<pair<int,int>>& points, int minX, int minY, int maxX, int maxY){
...
int main(void){
	...
	for(int it =0; it<TEST_CASES; it++){
		...
		vector<pair<int,int>> result = euclidan_to_display(points, p1.first, p1.second, p2.first, p2.second);

		for(int i =0; i<N; i++){
			cout << "("<<points[i].first<<"," << points[i].second<<") ---> "<< "("<<result[i].first<<"," << result[i].second<<")"<< endl; 
		}
		cout << endl;
	}
