...
int scc(int cur, vector<vector<int>>& adj, vector<int>& sccIds, vector<bool>& finished, vector<int>& discovered, int& sccCounter, int& vertextCounter, stack<int>& st) {
	int ret = discovered[cur] = vertextCounter++;
	st.push(cur);

	for (int next : adj[cur]) {
		if (discovered[next] == -1) {
			int subtree = scc(next, adj, sccIds, finished, discovered, sccCounter, vertextCounter, st);
			ret = min(ret, subtree);
...
// 같은 회의시간을 나타내는 두 정점이 같은 강결합요소에 속해 있늑 경우 답이 없다.
...
int main(void) {
	...
	for (int it = 0; it < test_cases; it++) {
		...
		cout << buf << " " <<  (solved.size() == 0 ? "IMPOSSIBLE" : "POSSIBLE") << endl;
