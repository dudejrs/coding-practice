...
int price(int m, int i) {
	return m + i + 1; 
}

int sink(int m, int p) {
	return m + p + 1;
}
...
vector<vector<Edge*>> initiailze(vector<int>& mileges, vector<int>& prices, vector<pair<int, int>>& availables) {
...
vector<pair<int, int>> getAvailables(int m, int p, function<bool()> is_available) {
...
int solve(vector<int>& mileges, vector<int>& prices, vector<pair<int, int>>& availables) {
	int m = mileges.size(), p = prices.size();
