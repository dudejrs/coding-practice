...
int getCandCoord(int y, int x, int*** hint, int* sum, int* length, int* known) {
  return getCandHint(hint[y][x][0], sum, length, known) &
         getCandHint(hint[y][x][1], sum, length, known);
}

void initialize(fstream& fd, int n, int** color, int** value, int*** hint,
                int& q, int* sum, int* length, int* known) {
  for (int i = 0; i < n; i++) {
    color[i] = new int[n];
  }
...
bool search(int n, int** color, int** value, int*** hint, int& q, int* sum,
            int* length, int* known) {
