...
int findCutVertex(int cur, bool is_root, vector<vector<int>>& adj, vector<int>& discovered, vector<bool>& is_cut_vertex, int& counter) {
	...
	for (int next : adj[cur]) {
		if (discovered[next] == -1) {
			...
			int subtree = findCutVertex(next, false, adj, discovered, is_cut_vertex, counter);
