...
void initialize(fstream& fd, vector<string>& name, vector<int>& volume,
                vector<int>& need, int& capcity) {
  string buf;
...
int pack(int capcity, int cur, vector<string>& name, vector<int>& volume,
         vector<int>& need) {
  ...
  if (capcity >= volume[cur]) {
    ret = max(ret, pack(capcity - volume[cur], cur + 1, name, volume, need) +
                       need[cur]);
  }
...
void getList(int capcity, int cur, vector<string>& answer, vector<int>& volume,
             vector<int>& need, vector<string>& name) {
  if (cur == name.size()) return;
  if (pack(capcity, cur, name, volume, need) ==
      pack(capcity, cur + 1, name, volume, need)) {
...
void solve(vector<string>& name, vector<int>& volume, vector<int>& need,
           const int capcity) {
