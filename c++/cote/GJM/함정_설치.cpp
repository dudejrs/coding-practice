...
int bipartite_match(vector<vector<bool>>& adj, vector<int>& aMatch,
                    vector<int>& bMatch) {
  int a = aMatch.size();
  int count = 0;
  for (int i = 0; i < a; i++) {
...
void set_trap(vector<bool>& aChosen, vector<bool>& bChosen,
              vector<string>& board) {
  int n = board.size(), m = board[0].size();
  vector<vector<int>> id(n, vector<int>(m, -1));
  int a = 0, b = 0;
...
void choose(vector<vector<bool>>& adj, vector<int>& aMatch, vector<int>& bMatch,
            vector<string>& board) {
  ...
  // 선택된 a와 b의 정점이 인점 한 경우, A의 정점을 선택해제하고, B의 대응된
  // 정점을 선택
  while (true) {
    bool changed = false;
