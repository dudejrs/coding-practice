...
int solve(const string& digits, const string& p, int m, string price,
          vector<bool>& taken) {
...
int solve2(const string& digits, const string& p, const int m, int idx,
           int less, int mod, int taken) {
  if (idx == digits.size()) {
  ...
  for (int i = 0; i < digits.size(); i++) {
    ...
    if (i > 0 && digits[i] == digits[i - 1] && ((taken & (1 << (i - 1))) == 0))
      continue;
