...
struct Node {
  ...
  Node(Node* parent, int value)
      : parent(parent), left(nullptr), right(nullptr), value(value) {}
...
struct BinaryTree {
  ...
  Node* _search(Node* cur, int value) {
    if (value < cur->value) return _search(cur->left, value);
    if (value > cur->value) return cur;
    return _search(cur->right, value);
    return cur;
  }

  Node* search(int value) { return _search(root, value); }
  ...
  void insert(Node* newer) { return _insert(root, newer); }
};
...
string solve(vector<int>& pre, vector<int>& in) { return ""; }
