...
inline int sink(int n, int m) {
	return n + m + 1;
} 

inline int project(int i) {
	return i + 1;
}

inline int item(int n, int i) {
	return n + i + 1;
}


vector<vector<int>> initialize(const vector<int>& project_profits, const vector<int>& purchasing_costs, const vector<vector<int>>& adj) {
...
int network_flow(int src, int sink, vector<vector<int>>& capacity, vector<vector<int>>& flow) {
	int v = capacity.size();
	int total_capcity = 0;
...
int solve(const vector<int>& project_profits, const vector<int>& purchasing_costs, const vector<vector<int>>& adj) {
	int n = project_profits.size();
	int m = purchasing_costs.size();
	int v = n + m + 2;

	vector<vector<int>> capacity = initialize(project_profits, purchasing_costs, adj);
	vector<vector<int>> flow(v, vector<int>(v, 0));

	int sum_of_profits = accumulate(begin(project_profits), end(project_profits), 0);
...
int main(void) {
	...
	for (int it = 0; it < test_cases; it++) {
		...
		cout << answer << " " << solve(project_profits, purchasing_costs, adj) << endl;
	}
