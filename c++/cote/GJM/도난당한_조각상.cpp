...
int source(int n) { return 0; }

int destination(int n) { return n - 1; }

bool _dfs(int cur, int dst, const vector<vector<int>>& adj,
          vector<bool>& visited) {
  if (cur == dst) {
    return true;
  }
...
vector<vector<int>> split_node(vector<vector<int>>& adj,
                               vector<int>& detectives) {
  int n = adj.size();
  ...
  while (true) {
    ...
    while (!q.empty()) {
      ...
      for (int next = 0; next < n; next++) {
        if (capacity[cur][next] - deployed[cur][next] > 0 &&
            parent[next] == -1) {
          parent[next] = cur;
          q.push(next);
        }
