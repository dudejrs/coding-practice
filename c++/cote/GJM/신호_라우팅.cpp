...
Graph initialize(int n, function<bool()> is_connected,
                 function<double()> cost_generator) {
  Graph ret;
...
int main(void) {
  ...
  auto cost_generator{bind(
      normal_distribution<float>{(MIN_C + MAX_C) / 2.0, (MAX_C - MIN_C) * 0.1},
      gen)};

  for (int it = 0; it < TEST_CASES; it++) {
    int n = gen() % (N - 3) + 3;
    int k = gen() % (n - 1) + 1;
    auto is_connected{bind(
        bernoulli_distribution{static_cast<double>((1.0 * M) / (n * (n + 1)))},
        gen)};
