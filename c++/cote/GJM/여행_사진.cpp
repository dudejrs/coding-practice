...
Location get_location(int y0, int x0, int y1, int x1, default_random_engine& gen) {
...
vector<Location> derived_from(vector<Area>& area, default_random_engine& gen) {
	...
	for (auto [l1, l2] : area) {
		ret.push_back({get_location(l1.first, l1.second, l2.first, l2.second, gen)});
	}
...
vector<vector<int>> get_adjacent(vector<Area>& area, vector<Location>& pictures) {
	int n = area.size();
...
bool dfs(int a, vector<int>& area_match, vector<int>& picture_match, vector<bool>& visited, vector<vector<int>>& adj) {
	if (visited[a]) {
		return false;
	}
	visited[a] = true;
	...
	for (auto p : adj[a]) {
		if (picture_match[p] == -1 || dfs(picture_match[p], area_match, picture_match, visited, adj)) {
			area_match[a] = p;
			picture_match[p] = a;
