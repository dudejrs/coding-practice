...
int dfs(int cur, vector<vector<int>>& graph, vector<bool>& finished,
        vector<int>& discovered, vector<int>& sccIds, stack<int>& st,
        int& vertexCounter, int& sccCounter) {
  ...
  for (const auto& next : graph[cur]) {
    if (discovered[next] == -1) {
      int subtree = dfs(next, graph, finished, discovered, sccIds, st,
                        vertexCounter, sccCounter);
      ret = min(ret, subtree);
