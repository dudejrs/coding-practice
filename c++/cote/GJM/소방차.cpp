...
int solve(int v, vector<vector<int>>& edges, vector<int>& spots, vector<int>& fire_stations) {
	...
	return accumulate(begin(spots), end(spots), 0, [&dist](int acc, int spot) {return acc + dist[spot - 1];});
}
