...
pair<vector<Road>, vector<Road>> initialize(int n, function<int()> cost, default_random_engine& gen) {
...
bool check(Road& road, vector<vector<int>>& adj) {
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			adj[i][j] = min(adj[i][j], min(adj[i][a] + cost + adj[b][j], adj[i][b] + cost + adj[a][j]));
		}
	}
