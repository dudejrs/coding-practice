...
int solve(int** map) { return path(0, 0, 0, map); }

int path2(int x, int y, int sum, int** map,
          int cache[N][N][MAX_VALUE * 100 + 1]) {
  if (y == N - 1) return sum + map[y][x];
  int& ret = cache[y][x][sum];
  if (ret != -1) return ret;
  sum += map[y][x];
  return (ret = max(path2(x, y + 1, sum, map, cache),
                    path2(x + 1, y + 1, sum, map, cache)));
}
...
int path3(int x, int y, int** map, int cache[N][N]) {
  ...
  if (ret != -1) return ret;
  return (
      ret = max(path3(x, y + 1, map, cache), path3(x + 1, y + 1, map, cache)) +
            map[y][x]);
}
