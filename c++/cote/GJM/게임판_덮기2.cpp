...
void search(const vector<string>& board, const vector<rotation>& rotations,
            const int H, const int W, int count) {
...
int solve(vector<string>& board, vector<string>& block, vector<int>& n) {
  ...
  for (int i = 0; i < n[0]; i++)
    for (int j = 0; j < n[1]; j++) covered[i][j] = (board[i][j] == '#' ? 1 : 0);
