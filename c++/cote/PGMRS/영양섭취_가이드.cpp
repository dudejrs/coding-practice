...
bool check(vector<bool>& visited, vector<vector<int>>& foods,
           vector<int>& goals) {
  int total_kcal = 0;
  ...
  for (int i = 0; i < foods.size(); i++) {
    ...
    if (total_carbonhydrate > goals[0] || total_fat > goals[2] ||
        total_kcal > goals_kcal)
      return false;
  }
...
int search(int cur, vector<bool>& visited, vector<vector<int>>& foods,
           vector<int>& goals) {
  if (cur == foods.size()) {
