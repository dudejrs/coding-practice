...
class RangeMinimumQuery :
	...
	def _init(self, data, left, right, cur) -> int :
		if left == right :
			...
			mid = (left + right) // 2
			left_min, right_min = self._init(data, left, mid, 2 * cur), self._init(data, mid + 1, right, 2 * cur + 1)
	...
	def update(self, idx: int, value: int) :
		...
		def update(cur_left: int, cur_right: int, cur: int) -> int :
			...
			if (cur_left == idx) :
				...
				left_min, right_min = update(cur_left, mid, 2 * cur), update(mid + 1, cur_right, 2 * cur + 1)
				self._range_min[cur] = min(left_min, right_min)
	...
	def query(self, left: int, right: int) -> int :
		...
		def search(cur_left: int, cur_right: int, cur: int) -> int:
			...
			mid = (cur_left + cur_right) // 2
			left_min, right_min = search(cur_left, mid, 2 * cur), search(mid + 1, cur_right, 2 * cur + 1)
