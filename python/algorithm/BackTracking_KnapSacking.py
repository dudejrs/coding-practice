...
def check_node(cur, cur_profit, cur_weight, items, W):
	...
	if cur == len(items) : return 
	
	if cur_weight < W and cur_profit > max_profit : 
		max_profit = cur_profit

	if not promising(cur, cur_profit, cur_weight, items, W) : return 
...
if __name__ == "__main__" :
	...
	for _ in range(10) :
		...
		items = [ (random.randint(0, MAX_PROFIT), random.randint(MIN_WEIGHT, MAX_WEIGHT) ) for _ in range(n)]
		W = MAX_WEIGHT * 1.5
