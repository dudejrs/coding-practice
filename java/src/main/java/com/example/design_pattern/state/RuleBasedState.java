...
public class RuleBasedState {
	...
	static {
		rules = Map.ofEntries(
		Map.entry(Trigger.CALL_DIALED, Map.of(
			State.OFF_HOOK, State.CONNECTING)),
		Map.entry(Trigger.HANG_UP, Map.of(
			State.CONNECTING, State.OFF_HOOK,
			State.ON_HOLD, State.OFF_HOOK)),
		Map.entry(Trigger.CALL_CONNECTED, Map.of(
			State.CONNECTING, State.CONNECTED)),
		Map.entry(Trigger.PLACE_ON_HOLD, Map.of(
			State.CONNECTED, State.ON_HOLD)),
		Map.entry(Trigger.TAKEN_OFF_HOLD, Map.of(
			State.ON_HOLD, State.CONNECTED)),
		Map.entry(Trigger.LEFT_MESSAGE, Map.of(
			State.CONNECTED, State.OFF_HOOK)),
		Map.entry(Trigger.STOP_USING_PHONE, Map.of(
			State.OFF_HOOK, State.ON_HOOK))
		);
	}
