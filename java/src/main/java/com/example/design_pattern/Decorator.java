...
public class Decorator {
  ...
  private static class Decorator_ {
    ...
    public static void main() {
      SMSDecorator decoratedNotifier = new SMSDecorator(new SlackDecorator(new ConcreteNotifier()));
    ...
    private static class TransparentShape implements Shape {
      ...
      public String render() {
        return String.format(
            "%s has %.2f%% transparency",
            shape.render(), ((double) Math.min(256, transparency) * 100) / 256);
      }
    }

    public static void main() {

      ColorShape decoratedShape = new ColorShape(new TransparentShape(new Circle(3.0), 128), "red");
  ...
  private static class StaticDecorator_ {
    ...
    private static class StaticDecorator<T extends Shape> implements Shape {
      ...
      protected static Object[] trimObjects(Object... args) {
        return Arrays.stream(args).skip(1).toArray();
      }
    ...
    private static class TransparentShape<T extends Shape> extends StaticDecorator {
      ...
      public String render() {
        return String.format(
            "%s has %.2f%% transparency",
            super.render(), ((double) Math.min(transparency, 256) * 100) / 256);
      }
    }
