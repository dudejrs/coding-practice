...
public class 조합탐색 {
  ...
  private static double solve3() {
    ...
    for (int i = 0; i < N; i++) {
      ...
      for (int j = 0; j < N; j++) {
        if (i != j) minEdge[i] = Math.min(minEdge[i], W[i][j]);
      }
    }
  ...
  private static double solve4() {
    ...
    for (int i = 0; i < N; i++) {
      ...
      nearest.add(l.stream().map((a) -> a.index).collect(toList()));
    }
  ...
  */
  ...
  private static boolean pathSwappingPruning() {
    ...
    List<Integer> item_list = IntStream.range(0, 4).mapToObj(i -> path.pop()).collect(toList());
    ...
    boolean ret =
        W[items[0]][items[1]] + W[items[2]][items[3]]
  ...
  private static double mstHeuristic(List<Edge> edges) {
    ...
    for (Edge e : edges) {
      ...
      if (s.merge(first, second)) ret += e.weight;
    }
  ...
  private static double solve6() {
    ...
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < i; j++) {
        if (W[i][j] != INF) edges.add(new Edge(i, j, W[i][j]));
      }
    }
  ...
  private static double solve7() {
    ...
    for (int i = 0; i < N; i++)
      for (int j = 0; j < CACHCE_DEPTH + 1; j++)
        for (int k = 0; k < (1 << N); k++) cache.put(List.of(i, j, k), -1.d);
