...
public class 알러지가_심한_친구들 {
  ...
  private static void searchSlow(int cur, List<Integer> edible, int chosen) {
    ...
    if (cur == m) {
      ...
      if (edible.stream().noneMatch(i -> i == 0)) best = Math.min(best, chosen);
      return;
    }
  ...
  private static void search(List<Integer> edible, int chosen) {
    ...
    OptionalInt first = IntStream.range(0, n).filter(i -> edible.get(i) == 0).findFirst();
  ...
  public static void main(String... args) throws IOException, ParseException {
    ...
    while (testCases > 0) {
      ...
      int[] nums =
          Arrays.stream(mf.parse(fd.readLine()))
              .map(String::valueOf)
