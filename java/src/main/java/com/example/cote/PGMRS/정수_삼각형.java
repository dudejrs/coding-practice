...
public class 정수_삼각형 {
  ...
  private static int max(int i, int j, List<List<Integer>> triangle) {
    ...
    return cache[i][j] =
        Math.max(max(i + 1, j, triangle), max(i + 1, j + 1, triangle)) + triangle.get(i).get(j);
  }

  private static int solve(List<List<Integer>> triangle) {
    for (int i = 0; i < MAX_HEIGHT + 1; i++) Arrays.fill(cache[i], -1);

    int height = triangle.size();

    for (int j = 0; j < height; j++) {
      cache[height - 1][j] = triangle.get(height - 1).get(j);
    }
