...
public class 교점에_별_만들기 {
  ...
  private static String[] solve() {
    ...
    for (int i = 0; i < l.size(); i++)
      for (int j = i + 1; j < l.size(); j++) {
        Point intersection =
            intersection(
                l.get(i)[0], l.get(i)[1], l.get(i)[2], l.get(j)[0], l.get(j)[1], l.get(j)[2]);
    ...
    for (int i = 0; i < result.length; i++) result[i] = new String(arr[i]);
  ...
  public static void main(String... args) throws IOException {
    ...
    while (testCases > 0) {
      ...
      for (int i = 0; i < n; i++) {
        l.add(Arrays.stream(rd.readLine().split(" ")).mapToLong(Long::parseLong).toArray());
      }
