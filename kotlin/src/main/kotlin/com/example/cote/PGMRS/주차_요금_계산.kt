...
data class Fee(
	...
	constructor(fees: Array<Int>) : this(fees[0], fees[1], fees[2], fees[3]){}
...
fun main() {
	...
	repeat(reader.readLine().toInt()) {
		val fees = reader.readLine().split(" ")
			.map(String::toInt)
			.toTypedArray()

		val numRecords = reader.readLine().toInt()

		val records = IntRange(1,numRecords)
			.map{
				reader.readLine()
			}
			.toTypedArray()
		...
		println("${answer.contentToString()} ${solve(fees, records).contentToString()}")
	}
}