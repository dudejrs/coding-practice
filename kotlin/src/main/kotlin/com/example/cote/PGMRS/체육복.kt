...
fun solve(n: Int, lost: List<Int>, reserve: List<Int>): Int {
	...
	val lostAndReserve = lost.toSet().run {
		intersect(reserve.toSet())
	}

	val lostAndNotReserve = lost.toSet().run{
		subtract(reserve.toSet())
	}
...
fun main() {
	val reader = with(Paths.get(System.getProperty("user.dir") + "/data/체육복.txt")) {
		Files.newBufferedReader(this)	
	}

	repeat(reader.readLine().toInt()) {
		val n = reader.readLine().toInt()
		val lost = reader.readLine().split(" ")
			.map(String::toInt)
		val reserve = reader.readLine().split(" ")
			.map(String::toInt)
	...
	repeat(TEST_CASES){
		...
		val lost = IntRange(1, n)
			.filter{
				random.nextFloat() < 0.5
			}
		val reserve = IntRange(1, n)
			.filter {
