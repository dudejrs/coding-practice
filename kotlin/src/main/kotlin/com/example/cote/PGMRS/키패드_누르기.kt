...
object 키패드_누르기 {
    ...
    fun getY(number: Int): Int = if (number == 0) 3 else (number - 1) / 3

    fun getX(number: Int): Int = if (number == 0) 1 else (number - 1) % 3
    ...
    fun solve(numbers: Array<Int>, prefered: String): String {
        ...
        return numbers.map { press(it, left, right) }.joinToString("")
    }
