...
object 상담예약제 {
    ...
    fun parseTime(time: String): Int {
        val tokens = time.split(":").map(String::toInt)
    ...
    fun solve(
        booked: Array<List<String>>,
        unbooked: Array<List<String>>
    ): List<String> {

        val bookedCustomers = ArrayDeque<Customer>()
        val unbookedCustomers = ArrayDeque<Customer>()

        bookedCustomers.addAll(booked.map { Customer(it[1], parseTime(it[0])) })

        unbookedCustomers.addAll(
            unbooked.map { Customer(it[1], parseTime(it[0])) })
    ...
    fun main() {
        ...
        repeat(reader.readLine().toInt()) {
            ...
            val booked =
                IntRange(1, numBooked)
                    .map { reader.readLine().split(" ") }
                    .toTypedArray()
            val numUnbooked = reader.readLine().toInt()
            val unbooked =
                IntRange(1, numUnbooked)
                    .map { reader.readLine().split(" ") }
                    .toTypedArray()
