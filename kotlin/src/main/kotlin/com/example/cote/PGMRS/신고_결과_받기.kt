...
fun solve(idList: List<String>, records: List<List<String>>, k: Int): List<Int> {

	val reportsById = mutableMapOf<String, MutableSet<String>>()
	val counts = mutableMapOf<String, Int>();
	val reports = records.map {
		Record(it[0], it[1])
	}

	for ((reporter, accused) in reports) {
		val accusedByReporter = reportsById[reporter] ?: mutableSetOf<String>()
	...
	return idList
		.map { 
			id -> reportsById[id]?.filter{
					it : String -> it in banned
				}?.size ?: 0
		}
}

fun main() {
	val reader = with(Paths.get(System.getProperty("user.dir") + "/data/신고_결과_받기.txt")){
		Files.newBufferedReader(this)
	}

	repeat(reader.readLine().toInt()) {
		val idList = reader.readLine().split(" ")
		val records = IntRange(1, reader.readLine().toInt())
			.map {
