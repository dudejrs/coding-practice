...
fun dfs(cur: Int, visited: MutableList<Boolean>, computers: List<List<Int>>) {
	visited[cur] = true
...
fun main() {
	...
	repeat(reader.readLine().toInt()) {
		...
		val computers = IntRange(1,numComputer)
			.map{
	...
	repeat(TEST_CASES) {
		val computers = List(N) { i ->
			val ret = MutableList(N) 
			{
				j -> if (i == j || random.nextInt(100) > 90) 1 else 0
