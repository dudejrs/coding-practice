...
object 유전_법칙 {
    ...
    fun Random.nextQueryLength(): Int = nextInt(MAX_QUERIES_LENGTH - 1) + 1
    ...
    fun solve(queries: List<Pair<Int, Int>>): List<String> {
        return queries.map { (generation, number) -> find(generation, number) }
    }

    fun main() {
        repeat(TEST_CASES) {
            val queries = List(random.nextQueryLength()) { random.nextQuery() }
