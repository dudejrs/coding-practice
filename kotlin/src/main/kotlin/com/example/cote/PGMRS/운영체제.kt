...
fun solve(programs: List<Array<Int>>): Answer {
	val sortedPrograms = programs.map {
		Program(it[0] - 1, it[1], it[2])
	}.sortedBy(Program::time)
	
	val q = ArrayDeque<Program>(sortedPrograms)

	val pq = PriorityQueue<Program>(compareBy(Program::priority, Program::time))
...
fun main() {
	val reader = with(Paths.get(System.getProperty("user.dir") + "/data/운영체제.txt")) {
		Files.newBufferedReader(this)
	}

	repeat(reader.readLine().toInt()) {
		val numPrograms = reader.readLine().toInt()
		val programs = MutableList(numPrograms) {
			Array<Int>(3){0}
		}
		val priorities = reader.readLine().split(" ")
			.map(String::toInt)
		val times = reader.readLine().split(" ")
			.map(String::toInt)
		val runtimes = reader.readLine().split(" ")
			.map(String::toInt)
		...
		val totalWaitedTimes = reader.readLine().split(" ")
			.map(String::toInt)
			.toTypedArray()
