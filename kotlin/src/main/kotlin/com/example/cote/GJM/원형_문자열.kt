...
object 원형_문자열 {
    ...
    class Comp(val group: List<Int>, val t: Int) : Comparator<Int> {
    ...
    fun initialize(random: Random): String {
        ...
        return buildString { repeat(n) { append(random.nextChar()) } }
    }

    fun getSuffixArray(target: String): List<Int> {
        var n = target.length
        var t = 1
        var group = MutableList(n) { target[it].code }.apply { add(-1) }
        ...
        while (t < n) {
            ...
            t *= 2
            val newgroup = MutableList(n) { 0 }.apply { repeat(t) { add(-1) } }
