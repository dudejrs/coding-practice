...
fun search(eater: List<List<Int>>, canEat: List<List<Int>>, edible: MutableList<Int>, n: Int, m: Int, chosen: Int){
...
fun solve(eater: List<List<Int>>, canEat: List<List<Int>>, n:Int, m:Int): Int {
	best = INF
...
fun main() {
	...
	val p = Paths.get(System.getProperty("user.dir")+"/data/알러지가_심한_친구들.txt")
	...
	repeat (reader.readLine().toInt()) {
		...
		repeat(m) {
			val token = reader.readLine().trim().split(" ")
				.drop(1)
				.map(person::indexOf)
